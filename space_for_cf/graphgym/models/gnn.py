...
class MCGNN(nn.Module):
    ...
    def forward(self, batch, edge_index=None, sample_bank = None, only_emb=False):
        # list embeddings of different components
        emb_list = []
        for module in self.children():
            if isinstance(module, GNN):
                batch = module(batch)
                emb_list.append(batch.node_feature) # embedding item user
        ...
        if self.training or cfg.model.eval_type != 'ranking':
            pred, true = self.post_mp(batch, edge_index=edge_index, sample_bank=sample_bank)
            return pred, true
        else:
            rating_matrix = self.post_mp(batch, sample_bank = sample_bank)
