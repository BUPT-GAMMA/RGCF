...
class Logger(object):
    def __init__(self, iter_total, num_labels, name='train', task_type=None):
        ...
        if task_type == None:
            ...
            elif cfg.dataset.task_type == 'ranking':
    ...
    def ranking(self):
        return {'recall': self._eval_metric}
    ...
    def update_stats_ranking_eval(self, lr, params, eval_metric=0):
        self._eval_metric = eval_metric
        self._params = params
        self._lr = lr
    ...
    def basic_ranking(self):
        return {'loss': round(self._loss / self._size_current, cfg.round),
                'lr': round(self._lr, cfg.round),
                'params': self._params,
                }

    def write_epoch_ranking(self, cur_epoch):
        eval_stats = self.ranking()
        epoch_stats = {'epoch': cur_epoch}
        if self.name == 'train':
            basic_stats = self.basic_ranking()
            stats = {**epoch_stats, **basic_stats}
        else:
            stats = {**epoch_stats, **eval_stats}
